...
export class ProductsService {
    ...
    if (createProductDto.barcode) {
      ...
      if (existing) {
        throw new ConflictException(
          'Ya existe un producto con este código de barras en tu tienda',
        );
      }
    }
  ...
  async update(
    ...
    if (
      ...
      const existing = await this.productsRepository.findOne({
        where: { tenantId, barcode: updateProductDto.barcode },
      });
      if (existing) {
        throw new ConflictException(
  ...
  async createMany(
    ...
    try {
      ...
      for (const dto of productsDto) {
        ...
        const product =
          (barcode && barcodeMap.get(barcode)) || nameMap.get(nameNormalized);
        ...
        if (product) {
          ...
          if (dto.cost !== undefined)
            product.cost = dto.cost !== null ? Number(dto.cost) : null;
          if (dto.stock !== undefined) product.stock = Number(dto.stock);
          if (dto.profitMargin !== undefined)
            product.profitMargin =
              dto.profitMargin !== null ? Number(dto.profitMargin) : null;
          if (dto.categoryId !== undefined) product.categoryId = dto.categoryId;
          if (dto.lowStockThreshold !== undefined)
            product.lowStockThreshold = Number(dto.lowStockThreshold);
          if (dto.sellByWeight !== undefined)
            product.sellByWeight = dto.sellByWeight;
